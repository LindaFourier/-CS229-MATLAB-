...
for i = 1:ite
    %***sigmoid function***%
    h_theta_x = 1./(1+  exp(-(train_mat*theta)) );
    %***使用梯度下降法更新模型系数***%
    theta(1,1) = theta(1,1) - (alpha/train_num) * sum( h_theta_x - train_y );
    theta(2,1) = theta(2,1) - (alpha/train_num) * sum( (h_theta_x - train_y).*train_x1 );
    theta(3,1) = theta(3,1) - (alpha/train_num) * sum( (h_theta_x - train_y).*train_x2 );
    %***计算cost***%
...
t = 0;   f = 0;
for i=1:size(test,1)
    h_t = 1/(1+  exp(-([1,train_x1(i),train_x2(i)]*theta)) );
    if h_t>=0.5
        label_test(i,1) = 1;
    else
        label_test(i,1) = 0;
...
for i = 1:size(x1vals,2)
    for j = 1:size(x2vals,2)
        x = [x1vals(i),x2vals(j)];
        h_t_plot = 1/(1+  exp(-([1,x1vals(i),x2vals(j)]*theta)) );
        if h_t_plot>=0.5
            Z(i,j) = 1;
        else
            Z(i,j) = 0;
